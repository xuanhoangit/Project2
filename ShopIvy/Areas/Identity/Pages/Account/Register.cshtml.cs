...
namespace ShopIvy.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {   
            [Required(ErrorMessage ="*{0} là không được để trống")]
            [AtLeastOneLetter(ErrorMessage ="*{0} chứa ký tự không hợp lệ")]
            [MaxLength(30,ErrorMessage ="*{0} không được dài quá 30 ký tự")]
            [MinLength(1,ErrorMessage ="*{0} không được ít hơn 1 ký tự")]
            ...
            [Required(ErrorMessage ="*{0} là không được để trống")]
            [Phone]
            //[PhoneNumber(ErrorMessage ="{0} không hợp lệ")]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            Input.Ho= Libraries.LibraryProject.StringTrimming(Input.Ho.Trim());
            Input.Ten=Libraries.LibraryProject.StringTrimming(Input.Ten.Trim());
            if (ModelState.IsValid)
            {
                var user = CreateUser();
                user.Ho=Input.Ho;
                user.Ten=Input.Ten;
                user.DiaChiDayDu=Input.DiaChiDayDu;
                user.NgaySinh=Input.NgaySinh;
                user.GioiTinh=Input.GioiTinh;
                user.PhoneNumber=Input.PhoneNumber;
                user.Tinh_ThanhPho=Input.Tinh_ThanhPho;
                user.Quan_Huyen=Input.Quan_Huyen;
                user.Phuong_Xa=Input.Phuong_Xa;
